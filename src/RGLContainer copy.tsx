...
const RGLContainer = forwardRef<HTMLDivElement, any>((props: any, ref) => {
  ...
  const mappedChildren = Children.map(children, (child: any) => {
    ...
    return React.cloneElement(<div data-key={key} data-grid={dataGrid} key={key}>{child}</div>);
  });

  return (
    <div ref={ref} style={{ ...style, position: 'relative',zIndex:999 }} className={className}>
      <ReactGridLayout
        {...props}
        className="layout"
        layout={layout}
